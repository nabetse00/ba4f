...
app_client = client.ApplicationClient(  # type: ignore
    client=algod_client, app=Bet(), signer=creator.signer,  # type: ignore
...
def get_boxes() -> list[tuple[int, bytes]]:
    ...
    # [{'name': '4E/UKYMZyJI7P59WufEfJVB+3E6d9zfWAdnx2dA1nxk='}]
...
# bet for acct1 10 Algo
place_bet(acct1, 10_000_000, 1)

# should fail
try:
    place_bet(acct1, 10_000_0000, 1)
except Exception:  #noqa
    print("second bet failed its ok !")

# bet for acct2 10 Algo
place_bet(acct2, 10_000_000, 2)

# bet for acct3 7 Algo
place_bet(acct3, 7_000_000, 1)

# increment bet by 10 algos
increment_bet(acct1, 10_000_000)
